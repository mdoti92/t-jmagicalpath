...
export class UIScene extends Phaser.Scene {
    ...
    setupListeners() {
        if (!this.room)
            return;
        this.room.state.listen('currentTurnPlayerId', (currentId) => {
        ...
        this.room.state.listen('diceValue', (val) => {
            if (val > 0) {
        ...
        this.room.state.listen('activeSymbolId', (symbolId) => {
            this.targetSymbolText.setText(`Objetivo:\n✨ Ficha #${symbolId}`);
        });
        this.room.state.listen('players', () => {
            this.renderScores();
        });
        this.room.state.listen('status', async (status) => {
            if (status === 'FINISHED') {
